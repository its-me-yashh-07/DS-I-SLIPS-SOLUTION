/*Slip_18_Q2. Write a program that multiply two single variable polynomials. Each
...
struct node* create(struct node *f)
 ...
 printf("\nEnter no of terms: ");
 scanf("%d",&n);
 printf("Enter term in descending order of power ");  f=(struct node *)malloc(sizeof(struct node));
 printf("\n Enter coeff: ");
 scanf("%d",&f->coeff);
 printf("\n Enter power: ");
 scanf("%d",&f->exp);
